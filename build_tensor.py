...
def build_timespan_tick(date, minutes=10, chunk_size = 100000) :
    ...
    # timespan으로 데이터 쪼개기
    print(f"Spiliting date: {date.strftime('%Y-%m')}")
    ...
    for chunk in chunks:
        ...
        os.makedir('timespan_tick', exist_ok = True)
...
def build_tensor_data(date, MAX_OBS_TICKS=2**15, TICK_FEAT_DIM=12):
    ...
    for t1, t2 in zip(tick_data, tick_data[1:]):
        try : 
            ...
            # macro placeholder
            macro = torch.zeros(16)   # 필요한 길이로 수정
